...
class DecisionTree:
    ...
    def _grow_tree(self, X, y, depth=0):
        ...
        feat_idxs = torch.randperm(n_feats)[:self.n_features]
    ...
    def _split(self, X_column, split_thresh):
        left_idxs = torch.nonzero(X_column <= split_thresh, as_tuple=True)[0]
        right_idxs = torch.nonzero(X_column > split_thresh, as_tuple=True)[0]
        return left_idxs, right_idxs

    def _entropy(self, y):
        hist = torch.bincount(y)
        ps = hist.float() / len(y)
        return -torch.sum(ps * torch.log(ps + 1e-9))

    def _most_common_label(self, y):
        labels, counts = torch.unique(y, return_counts=True)
        most_common = labels[torch.argmax(counts)]
